...
function createActions(actionDefs) {
    return actionDefs.map(function (actionDef, index) {
        if (actionDef.postback) {
            return MessageModel.postbackActionObject(actionDef.postback, null, { index: index, label: actionDef.postback });
        } else if (actionDef.url) {
            return MessageModel.urlActionObject(actionDef.url, null, 'http://oracle.com');
...
describe('MessageModel', () => {
    ...
    describe('TextConversationMessage', () => {
        it('Create and Validate Text', function () {
            const msg = MessageModel.textConversationMessage('May I help you?', createActions([{postback:'Yes'}, {postback:'No'}]));
    ...
    describe('AttachmentConversationMessage', () => {
        it('Create and Validate Attachment', function () {
            const msg = MessageModel.attachmentConversationMessage('image', 'https://cloud.oracle.com/opc/images/why-cloud-09.jpg', 
                    createActions([{url: 'Open'}, {call: 'Call'}, {share: 'Share'}, {location: 'Location'}]));
    ...
    describe('AttachmentConversationMessage Negative', () => {
        it('Create and Validate Invalid Attachment', function () {
            const msg = MessageModel.attachmentConversationMessage('unknown', 'https://cloud.oracle.com/opc/images/why-cloud-09.jpg', 
                    createActions([{url: 'Open'}, {call: 'Call'}, {share: 'Share'}, {location: 'Location'}]));
    ...
    describe('LocationConversationMessage and Channel Extensions', () => {
        it('Create and Validate Location', function () {
            const msg = MessageModel.locationConversationMessage(37.3435, -121.8887, 'Is it the right location?', null, createActions([{postback:'Yes'}, {postback:'No'}]));
    ...
    describe('PostbackConversationMessage and Global Actions', () => {
        it('Create and Validate Postback', function () {
            const msg = MessageModel.postbackConversationMessage({serious: true}, 'Seriously?');
            validateMessage(msg, 'postback');
            MessageModel.addGlobalActions(msg, createActions([{postback: 'Menu'}, {postback:'Bye'}]));
            validateMessage(msg, 'postback');
        });
    });
    ...
    describe('RawConversationMessage', () => {
        it('Create and Validate Raw', function () {
            ...
            const msg = MessageModel.rawConversationMessage({serious: true}, 'Seriously?');
