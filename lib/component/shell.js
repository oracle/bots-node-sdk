...
const { RestServiceContext } = require("../restservice/restServiceContext");
const entityUtil = require('../entity/utils');
const dataQueryUtil = require('../dataquery/utils');
const restServiceUtil = require('../restservice/utils');
...
function ComponentShell(config, registry) {
  ...
  return {
    ...
    getAllComponentMetadata: function () {
      ...
      if (registry.components) {
        for (var componentName in registry.components) {
          ...
          } else if (metadata.eventHandlerType==='RestService') {
            metadata.events = restServiceUtil.getRestServiceEventHandlers(component);
          }
    ...
    /**
     * Invokes the rest service event handler.
     * 
     * @private
     */
    invokeRestServiceEventHandler: function (componentName, requestBody, callback, sdkMixin) {
      // assert invocation callback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
      if (typeof callback !== 'function') {
        throw new Error('Invocation callback is required');
      }
      ...
      // Invoke handler
      try {
        const { component, context } = resolveInvocation(componentName, requestBody, RestServiceContext,sdkMixin);

        restServiceUtil.invokeRestServiceEventHandlers(component, context).then(() => {        
          callback(null, context.getResponse());
        }).catch(err => {
          if (!(err instanceof Error)) {         
            err = new Error(err);         
